...
class B_Model(BaseModel):
    ...
    def feed_data(self, LR_img, GT_img=None, kernel=None, ker_map=None):
    ...
    def optimize_parameters(self, step):
        ...
        srs, ker_maps, kernels = self.netG(self.var_L)
    ...
    def test(self):
        self.netG.eval()
        with torch.no_grad():
            srs, kermaps, kernels = self.netG(self.var_L)
            self.fake_SR = srs[-1]
            self.fake_ker = kernels[-1]
        self.netG.train()
    ...
    def test_x8(self):
        ...
        with torch.no_grad():
            sr_list = [self.netG(aug)[0][-1] for aug in lr_list]
